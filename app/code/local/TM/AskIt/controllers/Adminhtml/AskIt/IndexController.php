...
class TM_AskIt_Adminhtml_AskIt_IndexController extends Mage_Adminhtml_Controller_Action
{
    ...
    {
       return Mage::getSingleton('admin/session')->isAllowed('templates_master/askit');
    }
    ...
    public function editAction()
    {
        ...
        $model = Mage::getModel('askit/item')->load($id);
...
//            $id = $this->getRequest()->getParam('id');
            ...
            if (!empty($data['id'])) {
                ...
                $model
                    ...
                    ->setItemTypeId((int)$data['item_type_id'])
                    ->setItemId((int)$data['item_id'])
...
//                    ->setUpdateTime(now())
            ...
            try {
                ...
                if (!empty($data['new_answer_text'])) {
                    ...
                    $newAnswer = Mage::getModel('askit/item');
                    ...
                    Mage::dispatchEvent('askit_item_customer_notify_save_after', array(
                        'data_object' => $newAnswer)
                    );
                }
        ...
        if(!is_array($askitIds)) {
            ...
            try {
                foreach ($askitIds as $askitId) {
                    $askit = Mage::getModel('askit/item')->load($askitId);
                    $askit->delete();
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(
    ...
    public function massStatusAction()
    {
        ...
        if(!is_array($askitIds)) {
            ...
            try {
                ...
                foreach ($askitIds as $askitId) {
                    $askit = Mage::getSingleton('askit/item')
                        ->load($askitId)
                        ->setStatus($status)
