...
EwayPayment.prototype = {
    ...
    saveReviewWithEncryption: function () {
         ...
         */
        if (typeof checkout.validate == 'function') {
            if (checkout.validate()) {
                ...
                var paramsewayForm = Form.serialize(encryptedForm, true);
                var params = '';
                ...
                for (var i = 0; i < items.length; i++) {
                    ...
                    if (items[i].form == 'co-payment-form') {
                        if (jQuery("#p_method_ewayrapid_ewayone").is(':checked')) {
                            params += '&payment[cc_number]=' + paramsewayForm['payment[cc_number]'] + '&payment[cc_cid]=' + paramsewayForm['payment[cc_cid]'];
                        }
                ...
                params += '&method=' + jQuery('#checkout_method').val();
                params.save = true;
                var request = new Ajax.Request(
                        this.saveUrl,
                        {
            ...
            if (this.agreementsForm) {
                ...
                $insertinput = jQuery('.insertinput');
    ...
    saveReviewWithEncryption1: function () {
        if (checkout.loadWaiting != false)
            return;
        /*
         * Changed by Tran Trong Thang
