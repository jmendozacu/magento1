...
class Clarion_ReviewReminder_IndexController extends Mage_Core_Controller_Front_Action {
    ...
    function cronjobAction() {
        $products = Mage::getModel("clarion_reviewreminder/observer")->sendReviewReminder();
        print_r($products);
        exit();
    }
    ...
    function addReviewAction() {
        ...
        if ($this->_getSession()->isLoggedIn()) {
            ...
            $this->_getSession()->setBeforeAuthUrl(Mage::getUrl('review/product/list', array('id' => $productId, 'category' => $categoryId)));
