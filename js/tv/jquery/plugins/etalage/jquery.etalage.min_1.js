...
// NEW ELEMENTS & CACHE
                ...
                $container.addClass('fgc_etalage').show();
                var $thumbs;
                ...
                if (images > 1 || !o.smallthumb_hide_single) {
                    ...
                    var imgMain = $('ul.fgc-thumbnail > li:first .fancy-images').attr('href');
                    var arrImgMain = imgMain.split('/');

                    $.each($thumb_images, function () {
                        var $t = $(this);
                        src = $t.attr('datathumb');
                        ...
                        if ($thumb_images.length > 1) {
                            ...
                            var arrImgThumb = src.split('/');
                            if (arrImgMain[arrImgMain.length - 1] == arrImgThumb[arrImgThumb.length - 1]){
                                console.log (arrImgMain[arrImgMain.length - 1] , arrImgThumb[arrImgThumb.length - 1])
                                $('<li class="hide"><img class="etalage_small_thumb" src="' + src + '" /></li>').data('thumb_id', thumb_id).appendTo($smallthumbs_ul);
                            }else{
                                $('<li><img class="etalage_small_thumb" src="' + src + '" /></li>').data('thumb_id', thumb_id).appendTo($smallthumbs_ul);
                            }
                        }
                    ...
                    if (images > small_thumbs) {
                        ...
                        if (o.show_begin_end_smallthumb) {
                            src = $thumb_images.eq(images - 1).data('datathumb');
                            ...
                            src = $thumb_images.eq(0).data('datathumb');
...
// INITIATE FIRST RUN
                ...
                var $current_thumb = $thumbs.first().find('.etalage_thumb_image'),
                        ...
                        $src_zoom = $thumbs.first().find('.etalage_source_image').attr('src');
                if (o.magnifier_invert) {
...
//$thumbs.first().find('.zoomImg').attr('src', $src_zoom)
// FUNCTIONS
                ...
                var st_click = function ($next_active, moved) {
                    ...
                    $src_zoom = $next_thumb.find('.etalage_source_image').attr('src');
                    // Switch magnifier
                    if (o.magnifier_invert) {
                        $magnifier_img.attr('src', $current_thumb.data('src'));
                    }

                    var fgc11 = $next_thumb.height();
                    console.log(fgc11);
                    //$container.css('height',fgc11);
                    $container.css('max-height', 338);
                    //$container.parent().find('.etalage-control').css('top',fgc11+37);
                    //  alert($zoom_preview);
                    // Switch zoom image
                    if (preview) {
                        $zoom_preview.attr('src', $current_thumb.data('src'));
                    ...
                    //           $next_thumb.find('.etalage_thumb_image').attr('src', $src_zoom);
//                    $next_thumb.find('.zoomImg11').attr('src', $src_zoom);
                    // Reset autoplay
                    if (autoplay) {
                        stopAutoplay();
                        startAutoplay();
