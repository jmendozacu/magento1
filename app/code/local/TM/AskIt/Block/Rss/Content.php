...
class TM_AskIt_Block_Rss_Content extends Mage_Rss_Block_Abstract
{
    ...
    protected function _getProductId()
    {
        return $this->getRequest()->getParam('product_id',
            $this->getRequest()->getParam('productId', false)
        );
    }

    protected function _getCollection($productId = false, $questionId = false)
    {
        $collection = Mage::getModel('askit/item')
            ->getCollection()
            ...
            ;
        if ($productId) {
            $collection->addProductIdFilter($productId);
        }
//        if ($questionId) {
...
//        }
    ...
    protected function _toHtml()
    {
        ...
        $url = Mage::getUrl(Mage::helper('askit')->getRouteUrlPrefix());
        $productId = $this->_getProductId();
        $product = Mage::getModel('catalog/product')->load($productId);
        $title = Mage::getStoreConfig('askit/rss/title') .
        ...
        foreach ($items as $item) {
            ...
            $title = ($product ? '' : ':' . $product->getName()) . $question->getText()
                . ' Posted by ' . $question->getCustomerName();
            $url = Mage::getUrl(Mage::helper('askit')->getRouteUrlPrefix())
                . $product->getUrlPath();
            ...
            $data = array(
                ...
                'description'   => $item->getText()
                    . ' Posted by ' . $item->getCustomerName(),
