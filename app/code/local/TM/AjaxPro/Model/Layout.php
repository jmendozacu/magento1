...
//            'core/session',
        ...
        $status = !$collection->count(Mage_Core_Model_Message::ERROR)
            && !$collection->count(Mage_Core_Model_Message::WARNING)
            && !$collection->count(Mage_Core_Model_Message::NOTICE)
        ;

        $block->setStatus((bool)$status);
...
//        }
    ...
    {
        return $this->getLayout()->addOutputBlock($block, $blockName);
    }
