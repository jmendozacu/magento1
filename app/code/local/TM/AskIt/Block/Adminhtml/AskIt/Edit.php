...
class TM_AskIt_Block_Adminhtml_AskIt_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    public function __construct()
    {
        ...
        $this->_updateButton('reset', 'label', Mage::helper('askit')->__('Go to Related Item'));

        $data = Mage::registry('askit_data');

        $item = Mage::helper('askit')->getItem($data);
        $location = $item->getBackendItemUrl();

        $this->_updateButton('reset', 'onclick', "setLocation('{$location}')");
