...
class Plumrocket_SocialLogin_Model_Bitbucket extends Plumrocket_SocialLogin_Model_Account
{
	protected $_type = 'bitbucket';

    const URL_REQUEST_TOKEN = 'https://bitbucket.org/!api/1.0/oauth/request_token';
    const URL_AUTHORIZE = 'https://bitbucket.org/!api/1.0/oauth/authenticate';
    const URL_ACCESS_TOKEN = 'https://bitbucket.org/!api/1.0/oauth/access_token';
    const URL_ACCOUNT_DATA = 'https://api.bitbucket.org/2.0/user';

    protected $_responseType = array('oauth_token', 'oauth_verifier');

	protected $_fields = array(
					'user_id' => 'uuid',
		            'firstname' => 'displayname_fn',
		            'lastname' => 'displayname_ln',
		            'email' => 'email', // empty
		            'dob' => 'birthday', // empty
                    'gender' => 'gender', // empty
                    'photo' => 'avatar',
    ...
    protected $_popupSize = array(800, 600);
    ...
    protected function _prepareData($data)
    {
        ...
        if(!empty($data['display_name'])) {
            ...
            $data['displayname_fn'] = $nameParts[0];
            $data['displayname_ln'] = !empty($nameParts[1])? $nameParts[1] : '';
        }

        // Photo.
        if(!empty($data['links']['avatar']['href'])) {
           $data['avatar'] = $data['links']['avatar']['href'];
        }
