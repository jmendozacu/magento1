...
class VladimirPopov_WebForms_Model_Webforms extends VladimirPopov_WebForms_Model_Abstract {
    ...
    public function getEmailSmtpValidation() {
        if ($this->getData('email_smtpvalidation') > 0)
            return $this->getData('email_smtpvalidation') - 1;
            ...
            if ($field->getFieldsetId() == 0) {
                if ($all || $field->getType() != 'hidden') {
                    if ($field->getRequired())
                        $required_fields[] = 'field_' . $field->getId();
        ...
        foreach ($logic_rules as $logic)
            if ($logic->getIsActive())
                foreach ($logic->getTarget() as $target) {
                    $required = false;
                    if (in_array($target, $required_fields))
                        $required = true;
                    ...
                    foreach ($result->getData() as $key => $value) {
                        ...
                        if ($key == 'field_' . $field->getId() && $value && $field->getType() == 'subscribe') {
                            ...
                            foreach ($customer_email as $email) {
                                ...
                                /* add subscribe name */
                                //field ids: 20,12,29,43,56,66,75,84,93,102
                                $customer_name = '';
                                if (@$_POST['field']['20']) {
                                    $customer_name = self::splitFullname($_POST['field']['20']);
                                    $first_name = $customer_name[0];
                                    $last_name = $customer_name[1];
                                } elseif (@$_POST['field']['12']) {
                                    $customer_name = self::splitFullname($_POST['field']['12']);
                                    $first_name = $customer_name[0];
                                    $last_name = $customer_name[1];
                                } elseif (@$_POST['field']['29']) {
                                    $customer_name = self::splitFullname($_POST['field']['29']);
                                    $first_name = $customer_name[0];
                                    $last_name = $customer_name[1];
                                } elseif (@$_POST['field']['43']) {
                                    $customer_name = self::splitFullname($_POST['field']['43']);
                                    $first_name = $customer_name[0];
                                    $last_name = $customer_name[1];
                                } elseif (@$_POST['field']['56']) {
                                    $customer_name = self::splitFullname($_POST['field']['56']);
                                    $first_name = $customer_name[0];
                                    $last_name = $customer_name[1];
                                } elseif (@$_POST['field']['66']) {
                                    $customer_name = self::splitFullname($_POST['field']['66']);
                                    $first_name = $customer_name[0];
                                    $last_name = $customer_name[1];
                                } elseif (@$_POST['field']['75']) {
                                    $customer_name = self::splitFullname($_POST['field']['75']);
                                    $first_name = $customer_name[0];
                                    $last_name = $customer_name[1];
                                } elseif (@$_POST['field']['84']) {
                                    $customer_name = self::splitFullname($_POST['field']['84']);
                                    $first_name = $customer_name[0];
                                    $last_name = $customer_name[1];
                                } elseif (@$_POST['field']['93']) {
                                    $customer_name = self::splitFullname($_POST['field']['93']);
                                    $first_name = $customer_name[0];
                                    $last_name = $customer_name[1];
                                } elseif (@$_POST['field']['102']) {
                                    $customer_name = self::splitFullname($_POST['field']['102']);
                                    $first_name = $customer_name[0];
                                    $last_name = $customer_name[1];
                                }
                                file_put_contents(Mage::getBaseDir().'/webform.txt', 'te');
                                if (@$_POST['field']['20'] || @$_POST['field']['12'] || @$_POST['field']['29'] || @$_POST['field']['43'] || @$_POST['field']['56'] || @$_POST['field']['66'] || @$_POST['field']['75'] || @$_POST['field']['84'] || @$_POST['field']['93'] || @$_POST['field']['102']) {
                                    file_put_contents(Mage::getBaseDir().'/webform.txt', $email);
                                    $subscriber = Mage::getModel('newsletter/subscriber')->loadByEmail($email);
                                    $subscriber->setCustomerFirstname($first_name)->setCustomerLastname($last_name);
                                    $subscriber->save();
                                }
                                /* END add subscribe name */
                            }
                        }
                    }
                }
