...
EwayPayment.prototype = {
    ...
    saveReviewWithEncryption: function () {
        if (this.agreementsForm) {
            var agreementHolder = jQuery('#checkout-agreements');
            var error = false;
         ...
         * }
         */
        jQuery('#co-payment-form input[name="form_key"]').removeAttr('disabled');
        jQuery('#co-payment-form input[type="hidden"]').removeAttr('disabled');
        jQuery('#co-payment-form input[type="hidden"]').removeAttr('disable');
        jQuery('#co-payment-form input[name="form_key"]').removeAttr('disable');
        if (typeof checkout.getSteps == 'function') {
            var items = checkout.getSteps();
            for (stepCode = 0; stepCode < items.length; stepCode++) {
                if (items[stepCode]) {
        ...
        var item_special_ = jQuery('input[name="is-category-special"]').length;//special product
        if (item_special_) {
            var tvpass = jQuery('#register-customer-password input[name="billing[customer_password]"]').val();
            var tvpasscon = jQuery('#register-customer-password input[name="billing[confirm_password]"]').val();
            var is_login = jQuery('input[name="is_login"]').val();

            if (is_login != 'true' && !jQuery('#checkout_method').is(':checked') || (tvpass == '' && tvpasscon == '')) {
                jQuery('#checkout_method').prop('checked', true);
                ...
                checkout.setLoadWaiting('review');
                jQuery('#review-buttons-container button.opc-btn-checkout').attr('onclick', checkMedical());

                jQuery('form#co-payment-form').find('.insertinput').remove();
                jQuery('form#co-payment-form').append('<div class="insertinput"></div>');
                var $insertinput = jQuery('.insertinput');
                if (this.agreementsForm) {
                    jQuery('#checkout-agreements input').each(function () {
                        if (typeof jQuery(this)[0] != 'undefined') {
                            var inputAgreements = jQuery(this).clone();
                            inputAgreements.hide();
                            $insertinput.append(inputAgreements);
                        }
                    })
                }
                jQuery('#checkout-shipping-method-load input').each(function () {
                    if (typeof jQuery(this)[0] != 'undefined') {
                        var inputShipping = jQuery(this).clone();
                        inputShipping.hide();
                        $insertinput.append(inputShipping);
                    }
                })
                var ewayForm = new EwayForm();
                var formToEncrypt = ewayForm.findFormToEncrypt();
                var encryptedForm = ewayForm.encryptForm(formToEncrypt, false);
                var params = Form.serialize(encryptedForm, true);
                ...
                if (typeof checkout.getSteps == 'function') {
                    ...
                    for (var i = 0; i < items.length; i++) {
                        if (items[i]) {
                            paramsForm += '&' + Form.serialize(items[i].form);
                        }
                    }
                    paramsForm += '&method=' + jQuery('#checkout_method').val();
                    paramsForm = paramsForm.parseQuery();
                    jQuery.each(paramsForm, function (key, value) {
                        if (key == 'shipping_method' && params['shipping_method'] != '') {
                ...
                jQuery('.tvshowsubmitcheckout').show();
                var request = new Ajax.Request(
                        this.saveUrl,
                        {
                            method: 'post',
