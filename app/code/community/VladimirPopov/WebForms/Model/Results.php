...
class VladimirPopov_WebForms_Model_Results extends Mage_Core_Model_Abstract {
    ...
    public function addFieldArray($preserveFrontend = false, $field_types = array()) {
        ...
        foreach ($data as $key => $value) {
            if (strstr($key, 'field_')) {
                ...
                if ($field->getType() == 'select/checkbox' && !is_array($value))
                    $value = explode("\n", $value);
    ...
    public function sendEmail($recipient = 'admin', $contact = false) {
        ...
        if ($this->getWebformId() == 9) {
            if(isset($_POST['product_name']) && isset($_POST['product_link'])){
                $title_product_cate = 'Product name';
                $name_product_cate = $_POST['product_name'];
                $link_product_cate = $_POST['product_link'];
            }            
        }elseif($this->getWebformId() == 6){
            if(isset($_POST['category_name']) && isset($_POST['category_link'])){
                $title_product_cate = 'Category name';
                $name_product_cate = $_POST['category_name'];
                $link_product_cate = $_POST['category_link'];
            }
        }else {
            $title_product_cate = '';
            $name_product_cate = '';
        ...
        $vars = Array(
            ...
            'title_product_cate' => $title_product_cate,
            'name_product_cate' => $name_product_cate,
            'link_product_cate' => $link_product_cate,
        );
        ...
        if ($send_multiple_admin) {
            ...
            foreach ($email_array as $email) {
                ...
                if ($webform->getEmailAttachmentsAdmin())
                    foreach ($file_list as $file) {
                        ...
                        $mail->getMail()->createAttachment(
                                $attachment, Zend_Mime::TYPE_OCTETSTREAM, Zend_Mime::DISPOSITION_ATTACHMENT, Zend_Mime::ENCODING_BASE64, $file['name']
                        );
                    }
                ...
                if ($webform->getPrintAttachToEmail()) {
                    ...
                    $mail->getMail()->createAttachment(
                            $mpdf->Output('', 'S'), Zend_Mime::TYPE_OCTETSTREAM, Zend_Mime::DISPOSITION_ATTACHMENT, Zend_Mime::ENCODING_BASE64, 'result' . Mage::getSingleton('core/date')->date('Y-m-d_H-i-s', $this->getCreatedTime()) . '.pdf'
                    );
                }
                ...
                foreach ($fieldset['fields'] as $field) {
                    ...
                    if (strlen(trim($value)) && $field_visibility) {
                        if (!in_array($field->getType(), $options['skip_fields']) && $field->getResultDisplay() != 'off') {
                            ...
                            if ($field->getResultDisplay() != 'value')
