...
Checkout.prototype = {
    initialize: function (urls) {
        ...
        this.preSaveAllUrl = urls.preSaveAll;
    ...
    setStepResponse: function (response) {
                                ...
                                for (var elemIndex in arrValues) {
                                    if (typeof arrElements[elemIndex] != 'undefined') {
                                        ...
                                        if (typeof arrValues[elemIndex] != 'object')
                                            continue;
                                        // truong hop presave success full chay lai ajax presave mot lan nua
        ...
        if (response.nextstep != null) {
            var $nextstep = parseInt(response.nextstep) + 1;
            if ($nextstep < 4)
                checkout.preSaveNext($nextstep);
        }
        if (response.nextstepall != null) {
            var $nextstepall = parseInt(response.nextstepall) + 1;
            checkout.setLoadWaiting(false);
            if ($nextstepall < 4) {
                checkout.preSaveAll($nextstepall);
            } else {
                review.save();
            }
        }
    ...
    preSave: function (stepCode) {
        jQuery('#tvformkey').removeAttr('disabled');
    ...
    preSaveNext: function (stepCode) {
        ...
        var steps = '';
        if (items[stepCode]) {
            checkout.setLoadWaiting(items[stepCode].stepName);
            params += '&' + Form.serialize(items[stepCode].form);
        }
        if (items[stepCode].form == 'co-payment-form') {
            if (jQuery("#p_method_ewayrapid_ewayone").is(':checked')) {
                params += '&payment[cc_number]=' + jQuery("#ewayrapid_ewayone_cc_number").val() + '&payment[cc_cid]=' + jQuery("#ewayrapid_ewayone_cc_cid").val();
            }
        }
        ...
        params += '&steps=' + stepCode;
        params += '&nextstep=' + stepCode;
